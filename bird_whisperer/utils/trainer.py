...
def train(device, model, train_dataloader, test_dataloader, criterion, optimizer, unique_labels, start_epoch, n_epochs, save_model_path, rgs=None):
  best_macro_avg_f1 = 0
  best_epoch = 0
  train_losses = []
  test_epochs = []
...
def run_epoch(device, model, train_dataloader, criterion, optimizer, unique_labels, epoch):
  ...
  for batch_idx, batch in enumerate(tqdm(train_dataloader)):
    ...
    print(f"log_mels.shape: {log_mels.shape}, labels.shape: {labels.shape}")
  ...
  classification_report = classification_report(actual_labels, predicted_labels, labels=unique_labels, zero_division=0, output_dict=True)
  if "accuracy" not in classification_report:
    raise Exception("Accuracy not found in classification report. Ensure that 'accuracy' is present in the classification report.")
  
  return classification_report, avg_epoch_loss
...
def test_model(device, model, test_dataloader, criterion, unique_labels):
    with torch.no_grad():
      ...
      return classification_report, avg_test_loss
